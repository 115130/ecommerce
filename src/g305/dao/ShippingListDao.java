...
public class ShippingListDao extends BaseDao<ShippingList>{
    ...
    public List<ShippingList> getAllShippingList(){
        return queryAll("select * from Shipping_list oder by complete_data");
    }
    ...
    public ShippingList getOneShippingList(String oder){
        return queryAll("select * from Shipping_list where oder=? oder by complete_data",oder).get(0);
    }
    /**
     * 使用商品名模糊或者完整的名字查询订单
    ...
    public List<ShippingList> getNameShippingList(String productName){
        return queryAll("SELECT Shipping_list.product_id,Shipping_list.user_id,oder,address,money,satatus,complete_date FROM product,USER,shipping_list WHERE shipping_list.user_id = USER.id AND shipping_list.product_id = product.product_id " +
                "AND product.product_name like ? or product_name = ?","%"+productName+"%",productName);
    }
     ...
     * @return 添加了多少订单
     */
    public int addShippingList(int productId, int userId, String oder, String Address, double money, int status, Date complete_date){
     ...
     * @return 更改了多少行地址
     */
    public int updateAddressShippingList(String Address,int status,String oder){
     ...
     * @return 更改了多少行地址
     */
    public int updateOderShippingList(int status,String oder){
        return update("update update shipping_list set  status=? where oder=?",status,oder);
    }
    @Override
